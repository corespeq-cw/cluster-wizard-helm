...
{{- if and .Values.adminPass .Values.adminUser }}
UPDATE cluster_users SET password = {{ (index (splitList ":" (htpasswd "dummy" .Values.adminPass)) 1)  | squote }} WHERE username = {{ .Values.adminUser | squote }} AND (password IS NULL OR password = '');
{{- end }}
